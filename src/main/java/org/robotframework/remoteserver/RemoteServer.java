...
/**
 * Remote server for Robot Framework implemented in Java. Takes one or more test libraries and
 * exposes their methods via XML-RPC using an embedded web server. To use a different web server,
 * use {@link org.robotframework.remoteserver.servlet.RemoteServerServlet} instead.
 *
 * @see <a href="https://github.com/ombre42/jrobotremoteserver/wiki">jrobotremoteserver wiki</a>
 * @see <a href="http://code.google.com/p/robotframework/wiki/RemoteLibrary">Remote Library wiki
 *     page</a>
 * @see <a href="http://code.google.com/p/robotframework/wiki/UserGuide">User Guide for Robot
 *     Framework</a>
...
public class RemoteServer {
  ...
  public RemoteServer(String host, int port) {
    ...
    ServletContextHandler servletContextHandler =
        new ServletContextHandler(server, "/", false, false);
   ...
   * <ul>
   *   <li>start with a /
   *   <li>contain only alphanumeric characters or any of these: / - . _ ~
   *   <li>not end in a /
   *   <li>not contain a repeating sequence of /s
   * </ul>
   *
   ...
   * @return the previous library mapped to the path, or null if there was no mapping for the path
   */
   ...
   * @return the previous library associated with the path, or null if there was no mapping for the
   *     path.
   */
  ...
  /**
   * Gets a copy of the current library map. Keys in the map are the paths and the values are {@link
   * RemoteLibrary} wrappers of the libraries being served.
   *
  ...
  /**
   * A non-blocking method for stopping the remote server that allows requests to complete within
   * the given timeout before shutting down the server. New connections will not be accepted after
   * calling this.
   *
   * @param timeoutMS the milliseconds to wait for existing request to complete before stopping the
   *     server
   ...
   *
   * <ul>
   *   <li>Configure Log4J to log to the console
   *   <li>Set Log4J's log level to INFO
   *   <li>Redirect the Jetty's logging to Log4J
   *   <li>Set Jakarta Commons Logging to log to Log4J
   * </ul>
   *
   * This is convenient if you do not want to configure the logging yourself. This will only affect
   * future instances of {@link org.eclipse.jetty.util.log.Logger} and {@link
   * org.apache.commons.logging.Log}, so this should be called as early as possible.
   */
